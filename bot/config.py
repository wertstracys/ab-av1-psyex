...
class Config:
    def __init__(self):
        try:
            ...
            self.FFMPEG = config(
                "FFMPEG",
                default='ab-av1 auto-encode -i "{}" -e libsvtav1 --svt tile-rows=0:tile-columns=0:tune=2:enable-qm=1:qm-min=2:enable-variance-boost=1:tf-strength=1:sharpness=1 --keyint 321 --preset 9 --vfilter scale=1280:-2:flags=bicubic:param0=0:param1=1/2 --max-crf 70 --min-vmaf 84 -o "{}"',
            )
